...
\caption{Deployed watermarks under analysis--resynthesis at matched median PESQ. Cells: oriented AUC\,/\,TPR\,/\,\emph{achieved} FPR at the 1\%-FPR threshold fixed on the independent calibration split ($n=7157$ clean negatives). Outcomes: \emph{erasure} (AUC$\to$0.5, TPR$\to$0); \emph{calibration failure} (AUC retained, achieved FPR explodes on attacked clean audio); \emph{graceful degradation} (AUC and FPR retained, TPR reduced --- AudioSeal/DAC).}
