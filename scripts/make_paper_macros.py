...
def macros(e1: dict, e2: dict, e3: dict) -> list[str]:
    ...
    ev_w = e2["evaluation_auc_wave"]["pred_sensitivity"]
    ...
    lines = [
        ...
        # mel-probe (fixed mel-reading detector) — primary predictor result
        f"\\newcommand{{\\spearmanSens}}{{{f2(ev_m['phi_fit']['spearman'])}}}",
        ...
        f"\\newcommand{{\\spearmanSensWave}}{{{f2(ev_w['phi_fit']['spearman'])}}}",
        f"\\newcommand{{\\spearmanSNR}}"
        f"{{{f2(e2['evaluation_auc_mel']['pred_snr_db']['phi_fit']['spearman'])}}}",
        f"\\newcommand{{\\spearmanCentroid}}"
        f"{{{f2(e2['evaluation_auc_mel']['pred_spectral_centroid']['phi_fit']['spearman'])}}}",
        # necessity test: low-sensitivity decile deviation vs rest (both probes)
        f"\\newcommand{{\\necLowMel}}{{{f2(nec['auc_mel']['median_dev_low_sens'])}}}",
        f"\\newcommand{{\\necRestMel}}{{{f2(nec['auc_mel']['median_dev_rest'])}}}",
        f"\\newcommand{{\\necLowWave}}{{{f2(nec['auc_wave']['median_dev_low_sens'])}}}",
    ...
    # per-attacker within-Spearman (mel probe), for the E2 table
